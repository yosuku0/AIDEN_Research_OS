...
MCP host owns permissions, consent, connection lifecycle, and authorization. Client requests capabilities. Server exposes tools. Roots are advisory only. No secret passage through MCP. Tool invocation logging records trace_id, actor, tool, purpose, policy basis, approval reference, and result.

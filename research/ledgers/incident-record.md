...
| INC-003 | CI_SECURITY | H-003 | Secret scan in validate-governance.sh writes matched content to /tmp/aiden-secret-scan.txt and displays it in CI logs, risking information leakage. | HIGH | 2026-05-11 | Audit (Orchestrator) | open | Replace temp file with pipe processing; output file paths only (not matched content); add cleanup of temp files. | pending | validate-governance.sh and governance-ci.yml updated; awaiting Human review. | Human |
...
| INC-008 | PACKAGE_IMPORT | H-003 | Untracked source package artifact includes a fixture with a secret-like pattern. Repository fixture was sanitized, but the source package copy remains unsafe to commit. | HIGH | 2026-05-11 | Codex | open | Remove or sanitize the untracked package artifact before staging broad changes; keep secret scans outputting file paths only. | pending | Work stopped per CODEX_MASTER_PROMPT stop condition. Human review required before continuing package import. | Human |
